...
local sm_type "elastic_gbrl_compensation_fe"
...
import delimited "`project_dir'/output/elasticnet_wfd_1702_6_compensation_selected_weights_gbrl_fe.csv", clear varnames(1)
...
quietly count
if r(N) == 0 {
    di as error "No weighted variables found in `project_dir'/output/elasticnet_wfd_1702_6_compensation_selected_weights_gbrl_fe.csv"
    exit 2000
}
...
foreach v of local selected_vars {
    if !regexm("`v'", "^w_total_compensation_k1000_([0-9]+)$") {
        di as error "Unexpected variable format: `v' (expected w_total_compensation_k1000_#)"
        exit 111
    }
    local id = regexs(1)
...
* 2
regress w_xsalesmktrawCIQ `rhs_total_comp_var' i.bucket16, r
...
gen double bucket_fix = xb1 - _b[_cons] - _b[`rhs_total_comp_var']*`rhs_total_comp_var' if samp1
...
* 1
cnsreg w_xsalesmktrawCIQ `rhs_salary_var' bucket_fix if samp1, constraints(1) vce(robust)
...
post `posth' (1) ("`sm_type'") (_b[`rhs_salary_var']) (R2_c)
...
* 4
regress w_per_xsalesmktrawCIQ `rhs_per_total_comp_var' i.bucket16, r
...
* 3) Keep only the bucket16 contribution (NOT the constant, NOT salary)
gen double bucket_fix = xb1 - _b[_cons] - _b[`rhs_per_total_comp_var']*`rhs_per_total_comp_var' if samp1
...
* 3
cnsreg w_per_xsalesmktrawCIQ `rhs_per_salary_var' bucket_fix if samp1, constraints(1) vce(robust)
...
scalar R2_c = 1 - RSS/TSS
post `posth' (3) ("`sm_type'") (_b[`rhs_per_salary_var']) (R2_c)
...
scalar R2_c = 1 - RSS/TSS
post `posth' (7) ("`sm_type'") (_b[`rhs_salary_var']) (R2_c)
...
scalar R2_c = 1 - RSS/TSS
post `posth' (9) ("`sm_type'") (_b[`rhs_per_salary_var']) (R2_c)
...
save "..\output\wfd_regressions_by_type_24_elastic_gbrl_compensation_fe.dta", replace
