...
local sm_type "out_of_50_classes_elastic"
local elastic_salary_var "w_salary_k10_8_elastic"
local elastic_weight_var "w_weight_k10_8_elastic"
local elastic_total_comp_var "w_tcomp_k10_8_elastic"
local elastic_per_salary_var "w_per_salary_k10_8_elastic"
local elastic_per_weight_var "w_per_weight_k10_8_elastic"
local elastic_per_total_comp_var "w_per_tcomp_k10_8_elastic"

* Read elastic-net selected variables and weights
import delimited "..\output\elasticnet_wfd_1702_6_selected_weights.csv", clear varnames(1)
...
quietly count
if r(N) == 0 {
    di as error "No elastic-net weights found in ..\output\elasticnet_wfd_1702_6_selected_weights.csv"
    exit 2000
}
...
foreach v of local elastic_vars {
    if !regexm("`v'", "^w_salary_k50_([0-9]+)$") {
        di as error "Unexpected variable name in weights file: `v' (expected w_salary_k50_#)"
        exit 111
    }
    local id = regexs(1)
...
* 16 (replace w_per_total_compensation_k10_8)
...
save "..\output\wfd_regressions_by_type_2_elastic.dta", replace

di as text "Saved: ..\output\wfd_regressions_by_type_2_elastic.dta"
