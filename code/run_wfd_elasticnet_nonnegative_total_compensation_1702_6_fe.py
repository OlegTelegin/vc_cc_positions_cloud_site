...
def fit_absorbed_lasso(
    ...
    beta_orig : coefficients on original X scale, shape (p,)
    group_intercepts : per-group intercepts, shape (n_groups,)
                       NaN for groups not seen in training
    fallback_intercept : scalar intercept used if a group is unseen at prediction time
    """
    ...
    # Use scaling only (no centering) so coefficient recovery is simple.
    model = Pipeline(
        [
...
def predict_with_absorbed_fe(
    ...
    using the learned per-group intercepts. If a group was unseen in training,
    use fallback_intercept.
    """
    xb = X @ beta_orig
...
def main() -> None:
    ...
    data_path = project_dir / "data" / "temp_wfd_1702_6_w_certainty_quartiles.dta"
    ...
    fevar = "bucket16"
    xvars = [f"w_total_compensation_k50_{i}" for i in range(1, 54)]

    df = pd.read_stata(data_path).copy()

    # Match Stata splitsample behavior: split only complete cases for y + xvars + FE.
    ...
    # Build alpha grid off the within-transformed training y, so the scale reflects FE absorption.
    ...
    final_model, coef_orig, group_intercepts, fallback_intercept = fit_absorbed_lasso(
    ...
    selected_df.to_csv(
        output_dir / "elasticnet_wfd_1702_6_total_compensation_selected_weights_fe.csv", index=False
    )
    ...
    pred_df.to_csv(output_dir / "elasticnet_wfd_1702_6_total_compensation_test_predictions_fe.csv", index=False)
    ...
    with (output_dir / "elasticnet_wfd_1702_6_total_compensation_results_fe.txt").open(
        "w", encoding="utf-8"
    ) as fout:
        fout.write("Elastic net run on temp_wfd_1702_6_w_certainty_quartiles.dta\n")
        fout.write(f"Dependent variable: {yvar}\n")
        fout.write(f"Fixed effects (absorbed, unpenalized): i.{fevar}\n")
        fout.write("Predictors: w_total_compensation_k50_1-w_total_compensation_k50_53\n")
    ...
    print(f"Saved: {output_dir / 'elasticnet_wfd_1702_6_total_compensation_selected_weights_fe.csv'}")
    print(f"Saved: {output_dir / 'elasticnet_wfd_1702_6_total_compensation_test_predictions_fe.csv'}")
    print(f"Saved: {output_dir / 'elasticnet_wfd_1702_6_total_compensation_results_fe.txt'}")
