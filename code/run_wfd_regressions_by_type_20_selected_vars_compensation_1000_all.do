...
local sm_type "selected_vars_predict_salary_w_compensation_1000_all"
local rhs_salary_var "w_sal_k10_8_sel_comp1000a"
local rhs_weight_var "w_wgt_k10_8_sel_comp1000a"
local rhs_total_comp_var "w_tcmp_k10_8_sel_comp1000a"
local rhs_per_salary_var "w_psal_k10_8_sel_comp1000a"
local rhs_per_weight_var "w_pwgt_k10_8_sel_comp1000a"
local rhs_per_total_comp_var "w_ptcmp_k10_8_sel_comp1000a"

* Read selected variables list
import delimited "`project_dir'/data/selected_vars_predict_salary_w_compensation_1000_all.csv", clear varnames(1)

capture confirm variable selected_var
if _rc {
    capture confirm variable var
    if _rc {
        di as error "Selected-vars file is missing column: selected_var or var"
        exit 111
    }
    rename var selected_var
}

drop if missing(selected_var)
quietly count
if r(N) == 0 {
    di as error "No selected variables found in `project_dir'/data/selected_vars_predict_salary_w_compensation_1000_all.csv"
    exit 2000
}
...
save "`project_dir'/output/wfd_regressions_by_type_20_selected_vars_compensation_1000_all.dta", replace

di as text "Saved: `project_dir'/output/wfd_regressions_by_type_20_selected_vars_compensation_1000_all.dta"
