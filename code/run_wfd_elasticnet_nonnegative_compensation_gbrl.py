...
def center_data(
    ...
    """
    Center X and y, and return:
      X_centered, y_centered, X_means, y_mean
    """
    ...
    return X_centered, y_centered, X_means, y_mean
...
def build_alpha_grid(
    ...
    """
    Build an alpha grid from centered training data.
    """
    X_centered, y_centered, _, _ = center_data(X_train, y_train)
    ...
    n = max(len(y_centered), 1)

    scores = np.abs(X_scaled.T @ y_centered) / n
...
def fit_penalized_with_intercept(
    X_train: np.ndarray,
    y_train: np.ndarray,
    alpha: float,
    model_kind: str,
    ...
    """
    Fit lasso/elastic-net WITHOUT fixed effects, but WITH a standard intercept.

    We center X and y manually, fit with fit_intercept=False, then recover:
      intercept = y_mean - X_mean @ beta
    """
    X_centered, y_centered, X_means, y_mean = center_data(X_train, y_train)
    ...
    intercept = float(y_mean - X_means @ beta_orig)

    return model, beta_orig, intercept


def predict_with_intercept(
    X: np.ndarray,
    beta_orig: np.ndarray,
    intercept: float,
) -> np.ndarray:
    return X @ beta_orig + intercept
...
def cv_alpha_stats(
    ...
    for fold_idx, (fit_idx, val_idx) in enumerate(cv.split(X_train), start=1):
        ...
        for alpha_idx, alpha in enumerate(alphas, start=1):
            _, beta_orig, intercept = fit_penalized_with_intercept(
                X_fit,
                y_fit,
                alpha=float(alpha),
                model_kind=model_kind,
            ...
            y_hat = predict_with_intercept(X_val, beta_orig, intercept)
...
def evaluate_support_path_on_full_train(
    ...
    for i, alpha in enumerate(alphas, start=1):
        _, beta_orig, _ = fit_penalized_with_intercept(
            X_train=X_train,
            y_train=y_train,
            alpha=float(alpha),
            model_kind=model_kind,
...
def main() -> None:
    ...
    # Support-aware rule over the WHOLE alpha grid
    MIN_NONZERO_COEFS = 10           # set None to disable support floor
    ...
    # Split only complete cases for y + restricted xvars
    complete_mask = df[required_cols].notna().all(axis=1)
    complete_idx = df.index[complete_mask].to_numpy()

    if len(complete_idx) == 0:
        raise ValueError("No complete-case observations remain after applying y + restricted xvars.")
    ...
    log(
        f"    Complete-case rows: {n_complete} | train: {len(y_train)} | test: {len(y_test)}",
        VERBOSE,
    )
    ...
    log(f"    Base alpha from rule '{ALPHA_SELECTION}': {base_alpha:.6g}", VERBOSE)
    ...
    _, coef_orig, intercept = fit_penalized_with_intercept(
        X_train=X_train,
        y_train=y_train,
        alpha=final_alpha,
        model_kind=MODEL_KIND,
    ...
    log(f"    Final intercept: {intercept:.6g}", VERBOSE)
    ...
    selected_path = output_dir / "elasticnet_wfd_1702_6_compensation_selected_weights_nofe_supportaware.csv"
    selected_df.to_csv(selected_path, index=False)

    yhat_test = predict_with_intercept(X_test, coef_orig, intercept)
    ...
    pred_path = output_dir / "elasticnet_wfd_1702_6_compensation_test_predictions_nofe_supportaware.csv"
    ...
    results_path = output_dir / "elasticnet_wfd_1702_6_compensation_results_nofe_supportaware.txt"
    with results_path.open("w", encoding="utf-8") as fout:
        fout.write("Restricted penalized regression run (NO FE, support-aware alpha choice)\n")
        fout.write(f"Dependent variable: {yvar}\n")
        fout.write("Fixed effects: none\n")
        ...
        fout.write(f"Final intercept = {intercept:.10g}\n\n")
