...
def main() -> None:
    ...
    by_type_path = project_dir / "output" / "wfd_regressions_by_type_13_total_compensation_k1000_correlated_w_sm.dta"
    selected_vars_path = project_dir / "data" / "total_compensation_k1000_correlated_w_sm.csv"
    output_path = project_dir / "web" / "data" / "list_by_type_13.csv"
    fallback_sm_type = "total_compensation_k1000_correlated_w_sm"

    if not selected_vars_path.exists():
        raise FileNotFoundError(f"Missing file: {selected_vars_path}")

    sm_type = fallback_sm_type
    if by_type_path.exists():
        ...
        print(f"Warning: {by_type_path} not found; using fallback sm_type='{fallback_sm_type}'.")
    ...
    # Values look like w_per_salary_k1000_633; extract the k1000 numeric id.
    k1000_ids = (
